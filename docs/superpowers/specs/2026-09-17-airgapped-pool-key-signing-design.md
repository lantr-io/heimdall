...
**Revised 2026-09-22.** The three bindings this section first proposed – `bifrost_id_pk`, `deadline`, `submitter` – are replaced by one: a **nonce outpoint**, a UTxO under the operator's own payment key that the transaction must spend. The spec side is [REG-10] and [DRG-6] in `technical_documentation.md` (§SPO Registration, sections 4 and 7.1, rev 5.6); the contract side is the queued `spos-registry.ak` revision. The rest of this section is the reasoning as it now stands.
...
**Rollout.** The Config NFT is not redeployed: a governance `Update` rewrites field #9 (`spos_registry_policy_id`), and `treasury.ak` reads it at run time, so the treasury, the bridge state and every peg script stay. What is rebuilt: `spos-registry.ak` (new hash, new bootstrap outref, new root, new reference-script UTxO) and `spo-bans.ak`, which takes `registration_script_hash` as a compile parameter and moves with it (field #8). heimdall follows both fields per read (`src/cardano/config_params.rs`), so no TOML changes. Two constraints on the order. The identity trie keeps the old `bifrost_id_pk → pool_id` entries, the new [REG-5] demands their absence, and once field #9 has moved the old `Deregister` can no longer pass the treasury's [TSY-13] – so every pilot SPO exits under the old registry before the Config update and re-registers under the new one after it, each with a `v: 2` request file and one trip. And all of it inside one epoch, because the roster is the boundary snapshot of whichever list field #9 names. Two things worth folding into the same revision, since both scripts are rebuilt anyway: have `spo-bans.ak` read the registry hash from Config #9 the way `treasury.ak` does, so the next registry revision does not cascade; and a `Migrate` branch in the new registry that re-mints an existing binding against a membership proof, referencing the old node, which turns the migration into a permissionless re-mint with no cold-key trip.
