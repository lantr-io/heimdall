...
y_federation: <64 hex>   ← Config #11 / binocular bridge.y-federation-hex
share:        /var/lib/heimdall/federation-key.json (0600 — losing every copy loses the recovery path)
```

**Compare the `y_federation` values across all members before going further.** They must be
...
share it never regenerates: a re-run produces a *different* `Y_federation`, hence a different
treasury address, which the existing funds are not in. Below the threshold in surviving shares, the
recovery path is gone for good.
