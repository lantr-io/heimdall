...
- **`doctor` no longer prints flags you do not need.** Its unregistered-node and
  ...
  What `doctor` prints is now what the guide says: `--config <file> --submit`, nothing else, and
  `--help` marks the flag OPTIONAL instead of listing it like a required one.
