...
async fn test_sign_routes_serve_stored_json_and_gate_on_pool_id() {
    ...
    let state = make_shared_state();
    {
        ...
        s.own_pool_id_hex = pool_hex.clone();
        s.sign
            .insert((4, 1, 0, 0, RoundKey::Round1), r#"{"round":1}"#.to_string());
        s.sign
            .insert((4, 1, 0, 0, RoundKey::Round2), r#"{"round":2}"#.to_string());
    }
    let base = spawn_server(state).await;

    for (round, body) in [("round1", r#"{"round":1}"#), ("round2", r#"{"round":2}"#)] {
        let resp = reqwest::get(format!("{base}/sign/4/1/51/0/{round}/0/{pool_hex}.json"))
            .await
            .unwrap();
        assert_eq!(resp.status(), 200);
    ...
    let other = hex::encode([9u8; 28]);
    let resp = reqwest::get(format!("{base}/sign/4/1/51/0/round1/0/{other}.json"))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);

    // A session that was never published -> 404.
    let resp = reqwest::get(format!("{base}/sign/4/1/51/0/round1/1/{pool_hex}.json"))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);

    // The next OPPORTUNITY at the same session -> 404. The sequence is a path
    // segment, so a movement rebuilt at `B_2` cannot be served `B_1`'s blob even
    // though everything else about it is identical (WI-W8ZC4).
    let resp = reqwest::get(format!("{base}/sign/4/2/51/0/round1/0/{pool_hex}.json"))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);

    // The next ATTEMPT at the same opportunity -> 404 too. This is the one the
    // sequence cannot cover: a Round-2 shortfall re-runs Round 1 at the SAME
    // `B_i`, so serving attempt 0's commitments to attempt 1 would mix spent
    // nonces into a package built from fresh ones (WI-EJSVJ).
    let resp = reqwest::get(format!("{base}/sign/4/1/51/1/round1/0/{pool_hex}.json"))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);

    // A mode this node cannot sign in is a 404, not a segment to ignore: the
    // federation mode has no signing namespace at all, so nothing may be served
    // under one.
    let resp = reqwest::get(format!("{base}/sign/4/1/99/0/round1/0/{pool_hex}.json"))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);
}
...
async fn test_sign_endpoints_404_when_empty() {
    ...
    for round in ["round1", "round2"] {
        let resp = reqwest::get(format!("{base}/sign/0/1/51/0/{round}/0/{pool_hex}.json"))
            .await
            .unwrap();
        assert_eq!(resp.status(), 404);
    }
