...
/// The 36-byte on-chain outpoint encoding: `txid` in internal (consensus) byte order followed by
/// `vout` little-endian — the form a `PegOutDatum`'s `source_chain_treasury_utxo_id` and a
/// `TmDatum`'s `swept_peg_in_utxo_ids` both use, and what binocular's `pegout-request` writes.
#[must_use]
...
pub fn partition_by_pin(
    ...
    let expected = outpoint_bytes(treasury_outpoint);
    requests
        .into_iter()
...
mod tests {
    ...
    fn partition_by_pin_splits_on_the_datum_pin() {
        ...
        let matching = outpoint_bytes(&tip);
