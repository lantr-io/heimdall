...
//! see [`crate::epoch::signing::poll_sign_round`] for why FROST forces that.
//! A member of epoch N's roster who is absent at N+1 no longer stalls the
//! handoff.
//!
...
//! roster to survive INTO the incoming one. A boundary that drops more than
//! that — mass deregistration, a large pool banned — still stalls, and a node
//! sitting the epoch out cannot help even though it holds a share, because it
//! has no way to learn or validate the incoming group key it would be signing
//! over. Letting it contribute needs an attested-key path, not just a phase
//! change: a node that signs whatever `sig_msg` it is handed hands the treasury
//! to whoever asked. That is WI-078.
//!
//! A failed rotation is not fatal either way — no Update-Y is posted, the old
//! roster carries over, and the next boundary retries, exactly as the spec
//! prescribes for a failed ceremony. But note what carrying over MEANS after a
//! ban: the treasury stays under the old key, whose share set still includes
//! the banned member. Custody does not leave a misbehaving member until a
//! rotation completes.
...
async fn frost_sign_message(
    ...
    // WI-047: the rotation proceeds on a THRESHOLD subset of the outgoing
    // roster, fixed at the round-1 deadline. Without this, one member of epoch
    // N's roster who is not in epoch N+1's — banned, deregistered, stake-dropped
    // — stalls the handoff, and a permanent drop stalls it at every subsequent
    // boundary. That leaves the treasury under the OLD key, whose share set
    // still contains the member the ban was meant to remove.
    ...
    if !absent.is_empty() {
        crate::epoch_warn!(
            me,
            epoch,
            "Update-Y: signing the succession without {} outgoing member(s) — the rotation \
             still hands the treasury to the incoming key, and any absentee that is also out \
             of the incoming roster loses its share of the treasury as intended",
            absent.len()
        );
    }

    // S1 is fixed. Every signer must build this exact package or the binding
    // factors differ and the shares do not aggregate.
    ...
    poll_sign_round(peers, clock, config, ns, me, &s1, s1.len() + 1, &mut round2).await?;
...
mod tests {
    ...
    /// WI-047, the acceptance case: the rotation completes on a THRESHOLD subset
    /// of the outgoing roster while a member is absent, and the aggregated
    /// signature still verifies under the treasury's current key.
    ///
    /// This is the stall the item exists for. SPO 3 is in epoch N's roster and
    /// not in N+1's — banned, deregistered, stake-dropped — so it never reaches
    /// `PublishKeys` and never publishes a rotation payload. Before WI-047 the
    /// poll waited for it regardless and the handoff timed out at every
    /// subsequent boundary, leaving the treasury under the OLD key, whose share
    /// set still contains the member the ban was meant to remove.
    #[tokio::test]
    async fn a_threshold_subset_of_the_outgoing_roster_authorizes_without_the_absentee() {
        ...
        for id in keys.keys().copied().filter(|id| *id != absent) {
            ...
            let mut config = config_with(Some(per_node), None);
            config.identity.identifier = id;
            // Short enough that the test does not sit on the full quorum window
    ...
    /// roster is not authority to hand over the treasury, and the deadline
    /// closing on too few answers has to be a failure rather than a smaller
    /// signing set.
    #[tokio::test]
    async fn a_sub_threshold_subset_does_not_authorize() {
        ...
        assert!(
            ...
            "one of three must not authorize a 2-of-3 rotation, got: {err}"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
